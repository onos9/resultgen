...
import { clearSelectedProduct, changeProductAmount } from "../../store/actions/results.action";
import { IStateType } from "../../store/models/root.interface";

const OrderForm: React.FC = () => {
    const dispatch: Dispatch<any> = useDispatch();
    const selectedProduct: IResult | null = useSelector((state: IStateType) => state.results.selectedProduct);
    ...
    function saveForm(formState: IOrderFormState): void {
        if (selectedProduct) {
            if (selectedProduct.amount < formState.amount.value) {
                alert("Not enough results in warehouse");
                return;
            }

            formState.result.value = selectedProduct;
            dispatch(addOrder({
                id: 0,
                name: formState.name.value,
                amount: formState.amount.value,
                totalPrice: formState.totalPrice.value,
                result: formState.result.value as IResult
            }));
    ...
    function isFormInvalid(): boolean {
        return (formState.amount.error || formState.totalPrice.error
            || formState.name.error || formState.result.error || !formState.name.value
