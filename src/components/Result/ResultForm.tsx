...
import { IResult, ProductModificationStatus } from "../../store/models/result.interface";
import TextInput from "../../common/components/TextInput";
import { editProduct, clearSelectedProduct, setModificationState, addProduct } from "../../store/actions/results.action";
...
const ProductForm: React.FC = () => {
  ...
  const results: IResultState | null = useSelector((state: IStateType) => state.results);
  let result: IResult | null = results.selectedProduct;
  const isCreate: boolean = (results.modificationState === ProductModificationStatus.Create);
  
  if (!result || isCreate) {
    result = { id: 0, name: "", description: "", amount: 0, price: 0, hasExpiryDate: false, category: "" };
  }

  const [formState, setFormState] = useState({
    name: { error: "", value: result.name },
    description: { error: "", value: result.description },
    amount: { error: "", value: result.amount },
    price: { error: "", value: result.price },
    hasExpiryDate: { error: "", value: result.hasExpiryDate },
    category: { error: "", value: result.category }
  });
